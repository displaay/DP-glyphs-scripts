...
static NSString *DPVFStringValue(id value) {
	if ([value isKindOfClass:NSString.class]) {
		return value;
	}
	if ([value respondsToSelector:@selector(string)]) {
		id string = DPVFCall(value, @selector(string));
...
static UTF32Char DPVFCharacterAtIndex(NSString *string, NSUInteger *index) {
	unichar high = [string characterAtIndex:*index];
	(*index)++;
	if (CFStringIsSurrogateHighCharacter(high) && *index < string.length) {
...
static NSPoint DPVFTransformPoint(NSPoint point, NSAffineTransformStruct transform) {
	return NSMakePoint(
		point.x * transform.m11 + point.y * transform.m21 + transform.tX,
		point.x * transform.m12 + point.y * transform.m22 + transform.tY
	);
}

static BOOL DPVFTimingEnabled(void) {
	static NSInteger enabled = -1;
	if (enabled < 0) {
		const char *env = getenv("DPVF_TIMING");
		enabled = (env && strcmp(env, "0") != 0) ? 1 : 0;
	}
	return enabled == 1;
}

static void DPVFLogTiming(NSString *label, CFTimeInterval start) {
	if (DPVFTimingEnabled()) {
		NSLog(@"VFPreview %@ %.3f ms", label, (CACurrentMediaTime() - start) * 1000.0);
	}
}
...
@property (copy) NSString *lastSourceSignature;
...
- (NSArray<NSDictionary *> *)previewItems;
- (NSArray<GSLayer *> *)visibleLayers;
- (NSArray<NSDictionary *> *)sequentialPreviewItemsForLayers:(NSArray<GSLayer *> *)layers;
- (void)redrawLive;
- (void)refreshPanelForSourceChange;
@end
...
- (void)drawRect:(NSRect)dirtyRect {
	...
	NSArray<NSDictionary *> *items = [plugin previewItems];
	if (items.count == 0) {
		return;
	}
	...
	NSMutableArray<NSDictionary *> *drawItems = [NSMutableArray array];
	for (NSDictionary *item in items) {
		GSLayer *layer = item[@"layer"];
		...
		minX = MIN(minX, x);
		maxX = MAX(maxX, x + MAX(frame.width, 1.0));
		[drawItems addObject:@{@"frame": frame, @"x": @(x)}];
	}
	if (drawItems.count == 0 || minX == CGFLOAT_MAX) {
		return;
	}

	CGFloat width = MAX(maxX - minX, 1.0);
	CGFloat height = MAX(ascender - descender, 1.0);
	CGFloat padding = 12.0;
	CGFloat scale = MIN(
		(self.bounds.size.width - padding * 2.0) / width,
		(self.bounds.size.height - padding * 2.0) / height
	) * 0.95;
	if (scale <= 0.0) {
		return;
	}
	CGFloat originX = plugin.centerPreview ? (self.bounds.size.width - width * scale) / 2.0 - minX * scale : padding - minX * scale;
	CGFloat baselineY = (self.bounds.size.height - height * scale) / 2.0 + ascender * scale;

	for (NSDictionary *item in drawItems) {
	...
	DPVFLogTiming(@"panel draw", start);
}
...
- (NSString *)currentAxisSignature {
	...
	for (DPVFAxisRow *row in [self rows]) {
		[parts addObject:[NSString stringWithFormat:@"%@:%@:%lu", row.synthetic ? @"synthetic" : @"axis", row.axisId ?: @"", (unsigned long)row.index]];
	}
...
- (void)rebuild {
	...
	for (NSUInteger index = 0; index < count; index++) {
		...
		[slider sendActionOn:NSEventMaskLeftMouseDown | NSEventMaskLeftMouseDragged | NSEventMaskLeftMouseUp];
...
- (void)sliderChanged:(NSSlider *)sender {
	...
	DPVFLogTiming(@"slider callback", start);
}
...
- (void)scheduleActivationRefreshes {
	NSArray<NSNumber *> *delays = @[@0.0, @0.03, @0.12, @0.30];
	for (NSNumber *delay in delays) {
		__weak DPVFPreview *weakSelf = self;
		dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(delay.doubleValue * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
			[weakSelf refreshFromEditView];
		});
	}
}

- (void)refreshFromEditView {
	if (!self.active || self.liveDragging) {
		return;
	}
	[self ensureFontBound];
	if (!self.panel) {
...
- (NSString *)currentEditText {
	id<GSGlyphEditViewProtocol> graphicView = self.controller.graphicView;
	NSString *text = DPVFStringValue(graphicView.textStorage);
	if (text.length > 0) {
		return text;
	}
	GSFont *font = [self currentFont];
	text = DPVFStringValue(DPVFCall(font, @selector(currentText)));
	if (text.length > 0) {
		return text;
	}
	id tab = DPVFCall(font, @selector(currentTab));
	text = DPVFStringValue(DPVFCall(tab, @selector(string)));
	if (text.length > 0) {
		return text;
	}
	text = DPVFStringValue(DPVFCall(tab, @selector(text)));
	if (text.length > 0) {
		return text;
	}
	return nil;
...
- (GSLayer *)displayLayerForGlyph:(GSGlyph *)glyph master:(GSFontMaster *)master {
	if (!glyph) {
		return nil;
	}
	if (master) {
	...
	id layers = DPVFCall(glyph, @selector(layers));
	if ([layers respondsToSelector:@selector(allValues)]) {
		for (id layer in [layers allValues]) {
			if ([layer isKindOfClass:GSLayer.class]) {
				return layer;
			}
		}
...
- (NSArray<GSLayer *> *)layersFromDisplayString:(NSString *)displayString {
	GSFont *font = [self currentFont];
	if (!font || displayString.length == 0) {
		return @[];
	}
	GSFontMaster *master = [self activeMaster];
	NSCharacterSet *nameSeparators = [NSCharacterSet whitespaceAndNewlineCharacterSet];
	NSMutableArray<GSLayer *> *layers = [NSMutableArray array];
	NSUInteger index = 0;
	while (index < displayString.length) {
		GSGlyph *glyph = nil;
		unichar unit = [displayString characterAtIndex:index];
		if (unit == '/') {
			NSUInteger nameStart = index + 1;
			NSUInteger nameEnd = nameStart;
			while (nameEnd < displayString.length) {
				unichar nameUnit = [displayString characterAtIndex:nameEnd];
				if (nameUnit == '/' || [nameSeparators characterIsMember:nameUnit]) {
					break;
				}
				nameEnd++;
			}
			if (nameEnd > nameStart) {
				NSString *name = [displayString substringWithRange:NSMakeRange(nameStart, nameEnd - nameStart)];
				glyph = [font glyphForName:name];
			}
			index = nameEnd;
		}
		else {
			UTF32Char character = DPVFCharacterAtIndex(displayString, &index);
			glyph = [font glyphForCharacter:character];
			if (!glyph) {
				glyph = [font glyphForUnicode:[NSString stringWithFormat:@"%04X", character]];
			}
		}
		GSLayer *layer = [self displayLayerForGlyph:glyph master:master];
		if (layer) {
...
- (NSArray<GSLayer *> *)cachedVisibleLayers {
	id<GSGlyphEditViewProtocol> graphicView = self.controller.graphicView;
...
- (NSArray<GSLayer *> *)visibleLayers {
	NSArray<GSLayer *> *liveLayers = [self layersFromDisplayString:[self currentEditText]];
	if (liveLayers.count > 0) {
		return liveLayers;
	}
	return [self cachedVisibleLayers];
}

- (NSArray<NSDictionary *> *)previewItems {
	id<GSGlyphEditViewProtocol> graphicView = self.controller.graphicView;
	NSUInteger count = graphicView.cachedLayerCount;
	NSMutableArray<NSDictionary *> *cachedItems = [NSMutableArray array];
	NSMutableArray<GSLayer *> *cachedLayers = [NSMutableArray array];
	for (NSUInteger index = 0; index < count; index++) {
		GSLayer *layer = [graphicView cachedGlyphAtIndex:index];
		if (!layer) {
			continue;
		}
		[cachedLayers addObject:layer];
		NSPoint position = [graphicView cachedPositionAtIndex:index];
		[cachedItems addObject:@{@"layer": layer, @"x": @(position.x)}];
	}
	if (cachedItems.count > 0) {
		...
		BOOL monotonic = YES;
		for (NSDictionary *item in cachedItems) {
			GSLayer *layer = item[@"layer"];
			GSGlyph *glyph = layer.parent;
			DPVFFrame *frame = glyph ? [self frameForGlyph:glyph] : nil;
			CGFloat width = MAX(frame ? frame.width : layer.width, 1.0);
			CGFloat x = [item[@"x"] doubleValue];
			if (previousX != -CGFLOAT_MAX && x <= previousX + width * 0.2) {
				monotonic = NO;
			}
			previousX = x;
			minX = MIN(minX, x);
		...
		if (cachedItems.count == 1 || (monotonic && maxX > minX && (maxX - minX) > totalAdvance * 0.85)) {
			return cachedItems;
		}
		return [self sequentialPreviewItemsForLayers:cachedLayers];
	}
	return [self sequentialPreviewItemsForLayers:[self visibleLayers]];
}
...
- (NSArray<NSDictionary *> *)sequentialPreviewItemsForLayers:(NSArray<GSLayer *> *)layers {
	...
	CGFloat cursor = 0.0;
	for (GSLayer *layer in layers) {
		[items addObject:@{@"layer": layer, @"x": @(cursor)}];
		GSGlyph *glyph = layer.parent;
		DPVFFrame *frame = glyph ? [self frameForGlyph:glyph] : nil;
		cursor += MAX(frame ? frame.width : layer.width, 1.0);
	}
	return items;
}

- (NSString *)sourceSignature {
	NSString *text = [self currentEditText];
	if (text.length > 0) {
		return [@"text:" stringByAppendingString:text];
	}
	NSMutableArray<NSString *> *parts = [NSMutableArray array];
	for (GSLayer *layer in [self cachedVisibleLayers]) {
		GSGlyph *glyph = layer.parent;
		[parts addObject:glyph.name ?: [NSString stringWithFormat:@"%p", layer]];
	}
	return [@"layers:" stringByAppendingString:[parts componentsJoinedByString:@"/"]];
}

- (void)refreshPanelForSourceChange {
	if (!self.active || self.liveDragging || !self.panel.previewView) {
		return;
	}
	NSString *signature = [self sourceSignature] ?: @"";
	if ([self.lastSourceSignature isEqualToString:signature]) {
		return;
	}
	self.lastSourceSignature = signature;
	__weak DPVFPreview *weakSelf = self;
	dispatch_async(dispatch_get_main_queue(), ^{
		DPVFPreview *strongSelf = weakSelf;
		if (!strongSelf || !strongSelf.active || strongSelf.liveDragging || !strongSelf.panel.previewView) {
			return;
		}
		[strongSelf.panel.previewView setNeedsDisplay:YES];
		[strongSelf.panel.previewView displayIfNeeded];
	});
}
...
- (NSDictionary<NSString *, NSNumber *> *)syntheticInterpolations {
	...
	for (NSUInteger index = 0; index < masters.count; index++) {
		...
		[positions addObject:@{@"master": master, @"position": @([self syntheticAxisValueForMaster:master index:0 fallback:(CGFloat)index])}];
	}
	...
	return @{lowerMaster.id: @(1.0 - t), upperMaster.id: @(t)};
}
...
- (NSString *)cacheKeyForGlyph:(GSGlyph *)glyph {
	...
	return [NSString stringWithFormat:@"%@|%@|%.6f", name, [self axisCacheKey], [self sourceLastOperationForGlyph:glyph]];
}
...
- (GSLayer *)interpolatedLayerForGlyph:(GSGlyph *)glyph {
	...
	return [glyph interpolate:self.instance masters:masters keepSmart:NO smartSettings:nil layerName:nil additionalLayers:nil error:&error];
}
...
- (void)appendPathFrame:(DPVFPathFrame *)pathFrame toBezierPath:(NSBezierPath *)bezierPath {
	...
	for (NSNumber *number in order) {
		...
		if (pending.count >= 2) {
			[bezierPath curveToPoint:point controlPoint1:pending[pending.count - 2].pointValue controlPoint2:pending[pending.count - 1].pointValue];
		}
...
- (DPVFFrame *)frameForGlyph:(GSGlyph *)glyph {
	...
	DPVFLogTiming(@"interpolation", start);
	return frame;
}
...
- (void)drawNodeOverlaysForFrame:(DPVFFrame *)frame scale:(CGFloat)scale {
	...
	for (DPVFPathFrame *path in frame.paths) {
		for (NSUInteger index = 0; index < path.nodes.count; index++) {
			...
			NSMutableArray<NSNumber *> *neighborIndexes = [NSMutableArray array];
			if (index > 0) {
				[neighborIndexes addObject:@(index - 1)];
			}
			else if (path.closed && path.nodes.count > 1) {
				[neighborIndexes addObject:@(path.nodes.count - 1)];
			}
			if (index + 1 < path.nodes.count) {
				[neighborIndexes addObject:@(index + 1)];
			}
			else if (path.closed && path.nodes.count > 1) {
				[neighborIndexes addObject:@0];
			}
...
- (void)drawPreviewForLayer:(GSLayer *)layer options:(NSDictionary *)options {
	[self refreshPanelForSourceChange];
	if (!layer.parent) {
		return;
	}
	...
	[NSGraphicsContext restoreGraphicsState];
	DPVFLogTiming(@"edit draw", start);
}

- (void)drawBackgroundForLayer:(GSLayer *)layer options:(NSDictionary *)options {
	[self drawPreviewForLayer:layer options:options];
}
